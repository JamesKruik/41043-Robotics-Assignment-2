...
%1.1
%using IBVS to control camera mounted on end-effector
%camera sees a square target in the image with its corners located:
%p1Star = [662 362]; p2Star=[362 362]; p3Star=[362 662]; p4Star = [662 662];
%use the matrix form instead
pStar = [662 362 362 662; 362 362 662 662];

%cartesian coordinates of the targets corner
%P1 = [1.8, -0.25, 1.25]; P2 = [1.8, 0.25, 1.25]; P3 = [1.8, 0.25, 0.75]; P4 = [1.8, -0.25, 0.75];
%create 3D points instead
P=[1.8,1.8,1.8,1.8;
-0.25,0.25,0.25,-0.25;
 1.25,1.25,0.75,0.75];
...
%initial config or UR10
q0 = [pi/2; -pi/3; -pi/3; -pi/6; 0; 0];

%adding camera specifications
...
%Define the visual servoing gain (0 < lambda < 1) and a value for the depth
%gain of the controler
lambda = 0.6;
%depth of the IBVS
depth = mean (P(1,:));

%% 1.2 initialise simualtion (Display in 3D)
% Display UR10
Tc0= r.model.fkine(q0);
r.model.animate(q0');
drawnow

%locate the camera position to tip of end-effector
cam.T = Tc0;
...
%% 1.3  Initialise the simulation (Display in 2D image)
